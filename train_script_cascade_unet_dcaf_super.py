...
path = 'cascade-x5-unet16-dcaf-super-noiseless.pth' #<------------------------Path-to-the-cascade-wegihts----------------------
batch_size = 1
accum_val = 32
...
cascade = CascadeModule(net=torch.nn.ModuleList([UnetDCsuperAFModule(**model_kwargs).net for _ in range(5)]), **model_kwargs)
...
trainer = pl.Trainer(
    ...
    accumulate_grad_batches=accum_val,
    terminate_on_nan=True,
    default_root_dir='logs/CascadeUNetSuperAF_gaussian',
    callbacks=[
        pl.callbacks.ModelCheckpoint(
            save_last=True,
            save_top_k=4,
...
torch.save(cascade.net.state_dict(), 'cascade-x5-unet16-dcaf-super-gaussian.pth')
...
trainer = pl.Trainer(
    ...
    accumulate_grad_batches=accum_val,
    terminate_on_nan=True,
    default_root_dir='logs/CascadeUNetSuperAF_salt',
    callbacks=[
        pl.callbacks.ModelCheckpoint(
            save_last=True,
            save_top_k=4,
...
torch.save(cascade.net.state_dict(), 'cascade-x5-unet16-dcaf-super-salt.pth')
...
trainer = pl.Trainer(
    ...
    accumulate_grad_batches=accum_val,
    terminate_on_nan=True,
    default_root_dir='logs/CascadeUNetSuperAF_salt_normal_and_salt',
    callbacks=[
        pl.callbacks.ModelCheckpoint(
            save_last=True,
            save_top_k=4,
...
torch.save(cascade.net.state_dict(), 'cascade-x5-unet16-dcaf-super-normal-and-salt.pth')
