...
path = 'cascade-x5-unet16-dcaf-super-v4-noiseless.pth' #<------------------------Path-to-the-cascade-wegihts----------------------
batch_size = 1
accum_val = 32
...
cascade = CascadeModule(net=torch.nn.ModuleList([UnetDCsuperAFV4Module(**model_kwargs).net for _ in range(5)]), **model_kwargs)
...
trainer = pl.Trainer(
    ...
    accumulate_grad_batches=accum_val,
    terminate_on_nan=True,
    default_root_dir='logs/CascadeUNetSuperAFV4_gaussian',
    callbacks=[
        pl.callbacks.ModelCheckpoint(
            save_last=True,
            save_top_k=4,
...
torch.save(cascade.net.state_dict(), 'cascade-x5-unet16-dcaf-super-v4-gaussian.pth')
...
trainer = pl.Trainer(
    ...
    accumulate_grad_batches=accum_val,
    terminate_on_nan=True,
    default_root_dir='logs/CascadeUNetSuperAFV4_salt',
    callbacks=[
        pl.callbacks.ModelCheckpoint(
            save_last=True,
            save_top_k=4,
...
torch.save(cascade.net.state_dict(), 'cascade-x5-unet16-dcaf-super-v4-salt.pth')
...
trainer = pl.Trainer(
    ...
    accumulate_grad_batches=accum_val,
    terminate_on_nan=True,
    default_root_dir='logs/CascadeUNetSuperAFV4_salt_normal_and_salt',
    callbacks=[
        pl.callbacks.ModelCheckpoint(
            save_last=True,
            save_top_k=4,
...
torch.save(cascade.net.state_dict(), 'cascade-x5-unet16-dcaf-super-v4-normal-and-salt.pth')
