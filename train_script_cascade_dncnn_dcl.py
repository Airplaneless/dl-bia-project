...
path = 'cascade-x5-dncnn-dcl-noiseless.pth' #<------------------------Path-to-the-cascade-wegihts----------------------
batch_size = 8
accum_val = 16
...
cascade = CascadeModule(net=torch.nn.ModuleList([DnCNNDCLModule(**model_kwargs).net for _ in range(5)]), **model_kwargs)
...
trainer = pl.Trainer(
    ...
    accumulate_grad_batches=accum_val,
    terminate_on_nan=True,
    default_root_dir='logs/CascadeDnCNNDCL_gaussian',
    callbacks=[
        pl.callbacks.ModelCheckpoint(
            save_last=True,
            save_top_k=4,
...
torch.save(cascade.net.state_dict(), 'cascade-x5-dncnn-dc-gaussian.pth')
...
trainer = pl.Trainer(
    ...
    accumulate_grad_batches=accum_val,
    terminate_on_nan=True,
    default_root_dir='logs/CascadeDnCNNDCL_salt',
    callbacks=[
        pl.callbacks.ModelCheckpoint(
            save_last=True,
            save_top_k=4,
...
torch.save(cascade.net.state_dict(), 'cascade-x5-dncnn-dc-salt.pth')
...
trainer = pl.Trainer(
    ...
    accumulate_grad_batches=accum_val,
    terminate_on_nan=True,
    default_root_dir='logs/CascadeDnCNNDCL_salt_normal_and_salt',
    callbacks=[
        pl.callbacks.ModelCheckpoint(
            save_last=True,
            save_top_k=4,
...
torch.save(cascade.net.state_dict(), 'cascade-x5-dncnn-dc-normal-and-salt.pth')
